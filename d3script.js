window.onload = function() {
  d3.csv('./data/dvs_challenge_1_membership_time_space.csv').then(data => {
    visualizeData(data);
  });
  ...
  function visualizeData(data) {
    ...
    const groupedByDay = d3
      .nest() // middle step for further processing (daily mean)
      .key(d => d.date)
      .entries(data);
    ...
    const extentOfMeans = d3.extent(
      statsByDay.reduce((list, entry) => {
    ...
    const dailySignupsScaleWidth = d3
      .scaleLinear()
      ...
      meanCats.forEach(cat => {
        ...
        svg
          ...
          .on('mouseenter', function() {
            d3.select(`.${cat}-line`).style('stroke-width', 6);
          ...
          .on('mouseleave', function() {
            d3.select(`.${cat}-line`).style('stroke-width', 2);
            d3.selectAll(`.${cat}-dailyMeanText`)
              .transition()
              .duration(10)
              .style('fill-opacity', 0);
        ...
        svg
          ...
          .attr(
            'class',
            `dailyMeanText ${cat}-dailyMeanText dailyMeanText-${i}`
          )
          .attr('x', dayX + 5)
    ...
    d3.selectAll('.visualization-dailyMeanText').attr('dx', -35);
    ...
    const legendLines = d3
      .legendColor()
      .title('daily average')
      .scale(fillScale)
      .shapePadding(4);

    const legendBands = d3
      .legendSize()
      .title('daily signups')
      .labels(['20', '400', '750'])
